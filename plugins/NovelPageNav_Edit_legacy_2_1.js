...
function setupColumns(c) {
    ...
    if (isMobile()) {
        ...
        c.style.setProperty('column-width', Math.floor(c.clientWidth) + 'px', 'important');
        c.style.setProperty('column-gap', '30px', 'important');
    } else {
        const colWidth = Math.floor((c.clientWidth - 60) / 2);
...
function getSpreadWidth(c) {
    // 🌟 Math.floor로 내림 — 소수점 오차가 페이지마다 누적되어 왼쪽 글자가 잘리는 것 방지
    return Math.floor(c.clientWidth + (parseFloat(getComputedStyle(c).columnGap) || 60));
}
